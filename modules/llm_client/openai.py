    ...
    def analyze_code(self, code_snippet: str, context: Dict[str, Any]) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "error" in result:
                     log.error(f"OpenAI API returned error: {result['error']}")
                     raise requests.exceptions.RequestException(f"OpenAI API Error: {result['error']}")
                ...
                if attempt < max_retries - 1:
                    ...
                    log.error(f"Failed to communicate with OpenAI API after {max_retries} attempts: {e}")
                    raise
        
        log.error(f"OpenAI API failed after {max_retries} attempts (Rate Limit or Service Unavailable).")
        return ""
